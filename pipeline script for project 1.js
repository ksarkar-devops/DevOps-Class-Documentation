pipeline {
    ...
    stages {
		...
		stage('List files') {
            steps {
				sh '''
				    cd /var/lib/jenkins/workspace/DeployJavaProject/
				    ls -l
			    '''
            }
        }
		...
		stage('ACR Push') {
            steps {
                sh '''
				    docker tag java_proj ksarkar23987.azurecr.io/test-repo:test-tag
				    docker push ksarkar23987.azurecr.io/test-repo:test-tag
			    '''
            }
        }
    }
